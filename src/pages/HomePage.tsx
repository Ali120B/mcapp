...
export function HomePage(){
  ...
  return <section><h2>Welcome back</h2><h3>Recently played</h3>{instances.length?<ul className='cards'>{instances.slice(0,3).map(i=><li key={i.id}><b>{i.name}</b><p>{i.mc_version} · {i.loader}</p></li>)}</ul>:<EmptyState title='No instances yet. Create one from Library (Ctrl+N).'/>}<h3>Featured modpacks</h3><ul className='cards'>{packs.map(p=><li key={p.project_id}><b>{p.title}</b><p>{p.description}</p></li>)}</ul><h3>Featured mods</h3><ul className='cards'>{mods.map(p=><li key={p.project_id}><b>{p.title}</b><p>{p.description}</p></li>)}</ul></section>
}
